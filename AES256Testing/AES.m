...
+(NSString*)decrypt:(NSString*)cipherText withKey:(NSString*)key{
    ...
    int lenCipher =(int) [ciphertext length];
    int lenPass =(int) [key length];
    ...
    for (int i= 0; i < nBytes; i++) {
        pwBytes[i] = i < lenPass ? (int)[key characterAtIndex:i] : 0;
    }
    int length = 60;
    ...
    for (int i =0; i<8; i++) {
        counterBlock[i] = (int) [ctrTxt characterAtIndex:i];
    }
    ...
    for (int b=0; b<nBlocks; b++) {
        ...
        NSString *ctItem = [ct objectAtIndex:b];
        int lenCt =(int)[ctItem length];
        char* plaintxtByte= (char *)malloc(sizeof(char)*(lenCt + 1));
        //int i=0;
        for (int i=0; i<lenCt; i++) {
            // -- xor plaintext with ciphered counter byte-by-byte --
            int xorValue = cipherCntr[i] ^ (int) [ctItem characterAtIndex:i];
